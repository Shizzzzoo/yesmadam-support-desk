...
| 2026-06-24 | **Vercel demo UI shipped.** Hand-themed Vite/React "YesMadam Ops" frontend (blush/magenta, Bricolage/Hanken/Geist, wax-seal "Resolved by Assistant" stamp, booking before→after morph), seeded with a real pod snapshot. Live at https://yesmadam-ops.vercel.app. |
| 2026-06-25 | **Provider-coordination loop — designed, built, deployed, verified live.** Brainstorm→spec→plan→subagent-executed (`docs/superpowers/...provider-coordination-loop*`). New: `provider_responses` table; `bookings.provider_state` en-route lock (idle→alerted→en_route→stood_down); `professionals.contact_email`; functions `alert_provider`/`notify_provider`/`read_response`/`commit_en_route`/`offer_reschedule`/`apply_reschedule`/`stand_down`/`provider_stand_notice`/`sweep_provider_sla`; workflows `provider_reply` (on `provider_responses` UPDATE) + `sweep_sla` (cron); schedules `provider-reply` (DATASTORE) + `provider-sla` (TIME, 5-min SLA); `verify_service` now returns `provider_state`; `handle_ticket` gate gains an alert-pro branch + cross-ticket refund lock. **Verified live, all branches:** late→en_route+ETA, can't-make-it→replacement, on-site→human, reschedule accept/decline, SLA timeout→auto-resolve, late-reply guard, and the refund-lock (sub-cap refund on an en-route booking → parked for human, run `WAITING`). **Platform facts learned:** DATASTORE_EVENT workflows need their own schedule resource to fire; an existing ENUM column can't be extended on a live table (parked tickets reuse `triaged`); record write perm is `datastore.record.write`; integer col type is `INTEGER`; TIME schedule config `{cron}` targets a workflow; cross-fn call = `pod.functions.execute(name, dict)` + `function.execute` grant. |
| 2026-06-26 | **Free-runtime demo path confirmed + repo + frontend updated.** Inbound triage→alert verified end-to-end on the **free Claude Code daemon** runtime (no paid key): raw no-show ticket auto-triaged `no_show` → auto-alerted the pro. Earlier stalls were free-tier throttle under concurrent load, not bugs. Test artifacts cleaned off the pod (back to the 13/17/40 original seed). Whole project pushed to a **private GitHub repo**. Vercel frontend refreshed to **show the coordination loop** (Provider Coordination panel, en-route lock badges, SLA countdown, "pros coordinated" stat). |
| 2026-06-26 | **Gmail removed → in-pod notifications.** Managed `workspace-gmail` connector hit Google `redirect_uri_mismatch` on connect (managed-OAuth config issue, not fixable our side). `notify_customer`/`notify_provider` now record the message to `ticket_events` (reliable, no account); delivery channel is pluggable (Twilio/Resend API-key connector drops in later). |
| 2026-06-26 | **Concierge — conversational customer front door — designed, built, deployed, verified live.** Brainstorm→spec→plan→subagent-executed (`docs/superpowers/...concierge-conversational-front-door*`). New: agent `concierge` (toolsets POD + USER_INTERACTION; read-only on bookings/professionals/provider_responses, policy RAG; answer-only — never moves money); function `file_ticket` (hand-off → inserts a `tickets` row → the unchanged `handle_ticket` engine resolves within its gates); app `customer-chat` (slug `yesmadam-chat`, first-party SDK `client.conversations()` multi-turn). **Verified live e2e** (Tara Menon, manicure ₹699): 3-turn chat → context-matched the real booking → clarifying confirm → `file_ticket` fired autonomously → **booking cancelled + full ₹699 refund** → concierge read the result back truthfully. **Platform facts:** an agent needs BOTH `function.read` AND `function.execute` to call a function tool; an agent-invoked function runs under the AGENT's grants (so concierge needed `tickets` write — bookings stay read-only); missing grants surface as a `needs_approval` TOOL_CALL gate; instruction must forbid the approval step + re-filing on status checks. Docs (README/SUBMISSION) brought current with all three layers. |
